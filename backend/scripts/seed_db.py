...
PRODUCTS = [
    (
        ...
        189.00,
        62.00,
        "Waterproof shell jacket for trail use",
    ),
    (
        "SKU-77410",
        "Ridgeline Parka",
    ...
    (
        "SKU-33100",
        "Trailhead Daypack 22L",
        "accessories",
        79.00,
        24.00,
        "22-liter daypack with hydration sleeve",
    ),
    (
        "SKU-44201",
        "Cascade Rain Shell",
...
TICKETS = [
    (
        ...
        "NP-88213",
        "refund_request",
        "Jacket arrived with broken zipper",
        "resolved",
        "2026-06-06",
    ),
    (
...
def seed_data() -> None:
    """
    Insert the synthetic demo dataset.

    Tables with stable primary keys use INSERT OR REPLACE.

    The sales table is cleared before inserting the fixed seed dataset
    because its rows do not have a stable application-level primary key.
    This prevents duplicate revenue rows if seed_db.py is run again.
    """
    ...
    with get_conn() as conn:
        ...
        # The sales table uses generated row IDs rather than a stable
        # seed key. Clear the previous demo rows so rerunning the seed
        # script does not duplicate revenue and quantity totals.
        conn.execute(
            "DELETE FROM sales"
        )
...
def main() -> None:
    ...
    print(
        "CommerceOps AI seed setup completed successfully."
    )
